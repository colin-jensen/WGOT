...
void WGDarcyEquation<dim>::compute_hessian_determinant()
{
    ...
    const unsigned int dofs_per_cell   = fe.n_dofs_per_cell();
    const unsigned int n_q_points = fe_values_p.get_quadrature().size();

    FullMatrix<double> cell_matrix_M(dofs_per_cell_p, dofs_per_cell_p);
    FullMatrix<double> cell_matrix_E(dofs_per_cell_p, dofs_per_cell);
    FullMatrix<double> cell_matrix_D(dofs_per_cell_p, dofs_per_cell);
    ...
    {
        fe_values.reinit(cell);
        const typename Triangulation<dim>::active_cell_iterator cell_p =
                cell;
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
            for (unsigned int i = 0; i < dofs_per_cell_p; ++i)
            {
                ...
                {
                    cell_matrix_M(i, j) += fe_values_p[interior].value(i, q) *
                                           fe_values_p[interior].value(j, q) *
                                           fe_values_p.JxW(q);
                }

                for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                    cell_matrix_E(i, j) += fe_values_p[interior].value(i, q) *
                                           fe_values[interior].value(j, q) *
                                           fe_values.JxW(q);
                }
            }
